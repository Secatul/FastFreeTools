...
export default function MarkdownEditor() {
  ...
  // Insert text formatting into the markdown textarea
    ...
    <>
      ...
      <div className={`p-4 ${isFullScreen ? "fixed inset-0 z-50 bg-background" : "max-w-6xl mx-auto"}`}>
        ...
        <div className="flex flex-wrap mb-2 gap-2">
          <Button variant="outline" size="icon" onClick={() => handleFormatting("bold")} aria-label="Bold">
            <Bold className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => handleFormatting("italic")} aria-label="Italic">
            <Italic className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => handleFormatting("heading")} aria-label="Heading">
            <Heading className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => handleFormatting("unordered-list")} aria-label="Unordered List">
            <List className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => handleFormatting("ordered-list")} aria-label="Ordered List">
            <ListOrdered className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => handleFormatting("link")} aria-label="Link">
            <Link className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => handleFormatting("image")} aria-label="Image">
            <Image className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => handleFormatting("code")} aria-label="Code">
            <Code className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" onClick={handleFileExport} aria-label="Export Markdown">
            <Download className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="icon" component="label" aria-label="Import Markdown">
            <Upload className="h-4 w-4" />
            <input type="file" hidden onChange={handleFileImport} accept=".md,.markdown,text/markdown" />
          </Button>
          <Button variant="outline" size="icon" onClick={() => setIsFullScreen(!isFullScreen)} aria-label="Toggle Fullscreen">
        ...
        <main className="flex flex-col md:flex-row gap-4">
          ...
          <div className="w-full md:w-1/2 border rounded-md p-4 overflow-auto h-[calc(100vh-200px)] markdown-body">
            ...
            >
              {DOMPurify.sanitize(markdown)} {/* Sanitizing the Markdown content */}
            </ReactMarkdown>
          </div>
        </main>
